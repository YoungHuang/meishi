			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="orderItem.dish.label" default="Dish" /></th>
					
						<th><g:message code="orderItem.order.label" default="Order" /></th>
				...
				<g:each in="${orderItemInstanceList}" status="i" var="orderItemInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${orderItemInstance.id}">${fieldValue(bean: orderItemInstance, field: "dish")}</g:link></td>
					
						<td>${fieldValue(bean: orderItemInstance, field: "order")}</td>
